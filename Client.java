...
public enum Client {
    ...
    private void setupGUIListeners() {
        gui.getConnectButton().addActionListener(e -> {
            String username = gui.getUsernameField().getText().trim();
            String host = gui.getHostField().getText().trim();
            String port = gui.getPortField().getText().trim();
            ...
            if (connect(host, Integer.parseInt(port))) {
                sendClientName();
                gui.switchToChat(); // Method to switch to chat panel
            }
        });

        // Add listener for send button
        gui.getSendButton().addActionListener(e -> sendChatMessage());
        
        // Add listener for message field (Enter key)
        gui.getMessageField().addActionListener(e -> sendChatMessage());
    }

    private void sendChatMessage() {
        String message = gui.getMessageField().getText().trim();
        if (!message.isEmpty()) {
            if (!processClientCommand(message)) {
                if (isConnected()) {
                    sendMessage(message);
                    gui.getMessageField().setText(""); // Clear message field
                } else {
    ...
    // Modified process message to update GUI
    private void processMessage(long clientId, String message) {
        ...
        gui.appendToChatArea(String.format("%s: %s", name, message));
    }

    // Modified process room action to update GUI
    private void processRoomAction(long clientId, String clientName, String message, boolean isJoin) {
        if (isJoin && !knownClients.containsKey(clientId)) {
            ...
            gui.appendToChatArea(String.format("*%s joined the Room %s*", clientName, message));
            updateUserList();
        } else if (!isJoin) {
            ClientData removed = knownClients.remove(clientId);
            if (removed != null) {
                gui.appendToChatArea(String.format("*%s left the Room %s*", clientName, message));
                updateUserList();
            }
    ...
    private void updateUserList() {
        SwingUtilities.invokeLater(() -> {
            DefaultListModel<String> model = gui.getUserListModel();
            model.clear();
            for (ClientData client : knownClients.values()) {
                model.addElement(client.getClientName());
            }
        });
    }

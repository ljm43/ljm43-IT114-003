...
public class Room implements AutoCloseable{
    ...
    private String info(String message) {
            ...
            } else if (message.startsWith("/italic")) {
                return "_" + message.substring(7).trim() + "_";
            } else if (message.startsWith("/underline")) {
                return "__" + message.substring(10).trim() + "__";
            } else if (message.startsWith("/color")) {
                // Assuming a simple format: /color [color] [text]
                String[] parts = message.split(" ", 3);
                if (parts.length == 3) {
                    return "[" + parts[1] + "]" + parts[2];
                }
            }
            return message; // Return as is if no formatting applies
    }
    ...
    public int rollDice() {
        int result = new Random().nextInt(6) + 1; // Dice roll result between 1 and 6
   ...
   public String flipCoin() {
    ...
    public void sendDiceResult(ServerThread sender, int diceResult) {
        String message = String.format("%s rolled a %d.", sender.getClientName(), diceResult);
        System.out.println("Broadcasting dice roll result: " + message);
        
        // Broadcast the dice roll result to all clients in the room
        sendMessage(sender, message);
    }
    /**
     * Broadcasts the coin flip result to all clients in the room.
     *
     * @param sender    The client who initiated the coin flip.
     * @param flipResult The result of the coin flip.
     */
    public void sendCoinFlipResult(ServerThread sender, String flipResult) {
        String message = String.format("%s flipped a coin: %s.", sender.getClientName(), flipResult);
        System.out.println("Broadcasting coin flip result: " + message);

        // Broadcast the coin flip result to all clients in the room
        sendMessage(sender, message);
    }
